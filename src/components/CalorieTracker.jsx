...
const fetchFoods = async () => {
  ...
  const res = await fetch(
    `http://localhost:5000/foods?search=${searchKey}`
  );
  ...
  const fetchLogs = async () => {
    try {
      const res = await fetch(`http://localhost:5000/calorie-log/${user.id}`);
      const data = await res.json();

      setFoodList(data.map(item => ({
        name: item.food_name,
        meal: item.meal,
...
const addFood = async () => {
  ...
  try {
    const res = await fetch("http://localhost:5000/calorie-log", {
      method: "POST",
      headers: {
  ...
  const MealDropdown = () => {
    ...
    return (
      <div className="relative w-full">
        ...
        >
          <span>
            {meal ? `🍳 ${t(meal.toLowerCase())}` : t("selectMeal")}
          </span>
          <span
...
>
  ...
  value={
  selectedFood
    ? t(`foods.${getFoodKey(selectedFood.food_name)}`, selectedFood.food_name)
    : searchTerm
}
...
{showKeyboard && getLayout() && (
 <Keyboard
  layout={getLayout()}
  display={{
    "{bksp}": i18n.language === "hi"
      ? "⌫ हटाएं"
      : i18n.language === "te"
      ? "⌫ తొలగించు"
      : "⌫",

    "{space}": i18n.language === "hi"
      ? "स्पेस"
      : i18n.language === "te"
      ? "ఖాళీ"
      : "space"
                  ...
                  {foods.length > 0 ? (
                    foods.map((food) => (
                      <div
                        ...
                        className="px-3 py-2 hover:bg-orange-500 hover:text-white cursor-pointer"
                      >
                        {t(`foods.${getFoodKey(food.food_name)}`, food.food_name)} ({Math.round(food.calories)} kcal)
                      </div>
                    ))
                  ) : (
